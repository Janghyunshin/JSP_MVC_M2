...
public class MVCBoardDAO extends DBConnPool {
	...
	//목록 상세 검색 (Select ) : 주어진 일련 번호에 해당 하는 값을 DTO 에 담아 반환합니다. (한페이지 read)
	...
	public int updatePost (MVCBoardDTO dto) {
		...
		// 쿼리문 준비 
			...
			result = psmt.executeUpdate(); 	// update 성공 시  result 변수의 값이 > 0 
	...
	public int deletePost (String idx) {
		...
		try {
			...
			result = psmt.executeUpdate(); 	// result가 > 0 : 삭제 성공 , result = 0  : 삭제 실패 
	...
	// 비밀번호를 확인하는 메소드 (입력한 비밀번호가 DataBase의 값과 일치하는지 확인)
	public boolean confirmPassword (String pass, String idx ) {
		boolean isCorr = true ;
		try {
				 // COUNT(*) = 1 : 레코드가 존재하면 : client에서 넘긴 pass , idx DB에 존재 
			 	 // COUNT(*) = 0 : 레코드가 존재하지 않으면 : client에서 넘긴 pass, idx가 DB에 존재 X
